...
final class CodeHighlighter
{
    ...
    {
        $inner = (string) $this->highlightPhp($code);
        $header = '';
        if ($label !== null && $label !== '') {
            $header = '<div class="code-block__header"><div class="code-block__file">'
                . '<span class="code-block__label">' . htmlspecialchars($label, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</span>';
            if ($lang !== null && $lang !== '') {
                $header .= '<span class="code-block__file-note">' . htmlspecialchars($lang, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</span>';
            }
            $header .= '</div></div>';
        }

        return '<div class="code-block">' . $header
            . '<pre class="code-block__pre"><code class="code-block__code">' . $inner . '</code></pre></div>';
    }
